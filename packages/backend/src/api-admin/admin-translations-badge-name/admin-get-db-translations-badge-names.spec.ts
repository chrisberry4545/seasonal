...
describe('adminGetDbTranslationsBadgeNames', () => {
  ...
  beforeEach(async () => {
    ...
    result = await adminGetDbTranslationsBadgeNames(id);
  });

  test('calls getSqlQuery with the correct value', () =>
    expect(mockGetSqlQuery).toHaveBeenCalledWith(`${__dirname}/admin-get-db-translations-badge-names.sql`));
